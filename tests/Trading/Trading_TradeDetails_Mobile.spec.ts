...
test.describe('Mobile', async()=>{
    ...
    test(`${brand} Trade details test`, {tag:['@trading', '@mobile']}, async({page, AppNAGA}, testInfo)=>{
        ...
        await test.step(`Open new ${tradingInstrument} position`, async()=>{
            ...
            await instruments.openMobilePosition(tradingInstrument, 'BUY')
            await newPosition.installMobileLotsSize(90, 2)
            await newPosition.submitPosition()
        })
        await test.step(`Switch to My trades and open Trade details popup`, async()=>{
            await mainPage.openMobileMenu("My Trades")
            tradeInstrumentName = await myTrades.getMobileInstrumentName()
            deposit = await myTrades.getMobileDepositValue(currency)
            await myTrades.openMobileTradeDetails()
        })
    ...
    test(`${brand} Share opened position in feed. User shares posiiton from Trade details`, 
        {tag:['@trading', '@mobile', '@feed']}, async({page, AppNAGA}, testInfo)=>{
        ...
        await test.step('Open My accounts(header)-> social profile. Check post in user cabinet', async()=>{
            await mainPage.openMobileBackMenuPoint('My Social Profile')
            expect(await feed.checkMobileTradeBtnIsVisible()).toBeTruthy()
        })
        await test.step('Close opened positions, if they exist', async()=>{
            await mainPage.openMobileMenu('My Trades')
            await myTrades.closeMobilePositionsIfExist()
