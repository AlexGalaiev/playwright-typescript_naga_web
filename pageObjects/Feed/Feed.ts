...
export class Feed {
  ...
  constructor(page: Page) {
    this.page = page;
    this.mainPagePostText = page.locator("//div[@class='feed-status-post']").locator("//div[@class='feed-status-post__actions-input']");
  ...
  async deleteExistedPost(userName: string){
    await this.mainPagePostText.waitFor({state:'visible'})
    let post = await this.page.locator(".user-message-header__top",{hasText:userName});
  ...
  async deleteSharedPost(textDescription: string){
    await this.mainPagePostText.waitFor({state:'visible'})
    let post = await this.page.locator(".user-message-header",{hasText:textDescription});
    await post.locator(".user-message-header__options").click();
  ...
  async closeOpenedPost(userName: string){
    ...
    let exixtPost = await this.page.locator("[data-cy='options-dropdown-btn']")
    if(await exixtPost.isVisible()){
      await this.deleteExistedPost(userName);
    }else{}};
  ...
  async checkLike(userName: string){
    ...
    let popupLikes = await this.page.locator(".modal-content", {hasText: userName})
    let likedPerson = await popupLikes.locator(".feed-likes-modal__item__username")
  ...
  async addCommentToPost(userName: string, commentText: string){
    ...
    await postPopup.locator("#feed_comment_input_textarea").pressSequentially(commentText)
  ...
  async checkTextOfPost(userName: string){
    ...
    let text = await comment.locator(".feed-item-comment-list__item__comment").textContent()
    return text
  }
  async closeTab(){
  ...
  async addSharedTextToPost(testText: string){
    await this.page.locator("#feed_comment_input_textarea").pressSequentially(testText)
